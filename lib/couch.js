...
/**
 * Makes a HTTP request to Couch as the user associated with the provided
 * req object
 */

/*
exports.requestAs = function (config, req, method, path, data, opt, callback) {
    if (!callback) {
        callback = opt;
        opt = {};
    }
    ...
    opt = opt || {};
    opt.Cookie = req.headers.cookie;
    var loc = url.resolve(config.couch.url, path);
    console.log(['request', method, loc, data, opt, callback]);
    return couchr.request(method, loc, data, opt, callback);
};
*/
